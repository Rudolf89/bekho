...
test('el planner es transversal (catálogo compartido, sin academia)', function () {
    // La tabla no tiene academia_id: mismas filas con o sin tenant.
    expect(PlannerCiclo::count())->toBeGreaterThan(0)
        ->and(PlannerCiclo::first()->getAttributes())->not->toHaveKey('academia_id');
});
